        ...
        public int[][] directions = new int[][]
{
        ...
        public List<int[]> BFS_PathFind(VarosElem[,] varos, int targetX, int targetY)
        {
            Tuple<int, int> target = new Tuple<int, int>(targetX, targetY);

            Queue<(int, int, List<int[]>)> bfsQueue = new Queue<(int, int, List<int[]>)>();
            bool[,] visitedNodes = new bool[varos.GetLength(0), varos.GetLength(1)];
            ...
            while (bfsQueue.Count > 0)
            {
                ...
                foreach (var move in directions)
                {
                    ...
                    if (newPosX >= 0 && newPosX < varos.GetLength(0) && newPosY >= 0 && newPosY < varos.GetLength(1) && !visitedNodes[newPosX, newPosY])
                    {
                        ...
                        if (varos[newPosX, newPosY] is Ground)
                        {
